...
class DisFitProblem(object):
    ...
    def __init__(self, yaml_path, n_dis=1000, fold_change=None):
        """        
        Args:
            yaml_path (:obj:`str`): path to PEtab `yaml` file
            n_dis (:obj:`int`, optional): number of time discretiaation steps
    ...
    def yaml_path(self):
        ...
        Returns:
            :obj:`str`: path to PEtab `yaml` file
        """
    ...
    def yaml_path(self, value):
        ...
        Args:
            value (:obj:`str`): path to PEtab `yaml` file
        
        Raises:
            ValueError: if yaml_path is not a yaml file or corresponding PEtab problem specification is flawed
        """
        if !isinstance(value, string) or !value.endswith('.yaml') or !value.endswith('yml'):
            raise ValueError('`yaml_path` must be a path to a yaml file')
        result = subprocess.run(['petablint', '-v', '-y', value])
        if result.returncode != 0:
            raise ValueError('Incorrect PEtab problem specification.')
    ...
    def n_dis(self):
        ...
        Returns:
            :obj:`int`: number of time discretization steps
        """
        return self._n_dis

    @n_dis.setter
    def n_dis(self, value):
        """Set n_dis
        ...
        Raises:
            ValueError: if n_dis is not an integer > 5
        """
        if !isinstance(value, int) or (n_dis < 6):
            raise ValueError('`n_dis` must be an integer > 5.')
        self._n_dis = value
    ...
    def _set_julia_code(self):
        ...
        sbml_file = os.path.join(os.path.dirname(yaml_path), sbml_file)
        ...
        generated_code.extend(bytes('n_dis = {} # Setting number of ODE discretisation steps\n'.format(self.n_dis), 'utf8'))

        generated_code.extend(bytes('\n', 'utf8'))  
        generated_code.extend(bytes('# Data\n', 'utf8'))
        generated_code.extend(bytes('data_file_path = joinpath(pwd(), "tests", "fixtures", "G2M_copasi.csv")\n', 'utf8'))
        generated_code.extend(bytes('df = CSV.read(data_file_path)\n', 'utf8'))
        generated_code.extend(bytes('t_exp = Vector(df[!, :t]) # set of simulation times.)\n', 'utf8'))
        generated_code.extend(bytes('t_sim = range(0, stop=t_exp[end], length=n_dis\n', 'utf8'))
        ...
        generated_code.extend(bytes('@variable(m, aux)\n', 'utf8'))
        generated_code.extend(bytes('numerical_error_penalty = ', 'utf8'))
        penalties = []
        for variable in variables:
            penalties.append('sum(weight*({}[k] - {}[k-1])^pow for k in 2:length(t_sim))\n'.format(variable, variable))
        generated_code.extend(bytes('    + '.join(penalties), 'utf8'))
        generated_code.extend(bytes('@constraint(m, aux == numerical_error_penalty)\n\n', 'utf8'))
        ...
        generated_code.extend(bytes('    + aux\n    )\n\n', 'utf8'))
